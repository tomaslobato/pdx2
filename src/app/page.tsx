...
export default function Home() {
  ...
  const askAI = async (ev: FormEvent) => {
    ...
    const genAI = new GoogleGenerativeAI("AIzaSyBOM_iK9LuQ_DVu6vv1kxTV8AkReQ4kyGI")
  ...
  return (
    <main className="max-w-2xl m-auto flex flex-col">
      ...
      <form action="" className="text-xl flex fixed bottom-0 p-3 border-zinc-500 border gap-1.5 bg-black" style={{ width: 672, maxWidth: "100%" }}>
        <input type="text" className="px-3 rounded-lg w-full" placeholder="Prompt..." onChange={(ev) => setPrompt(ev.target.value)} />
        <button onClick={(ev) => askAI(ev)} className="px-4 py-2 rounded-lg bg-zinc-900">ask</button>
      </form>
    </main >
  )
}
